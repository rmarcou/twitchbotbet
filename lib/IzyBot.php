...
class IzyBot {
    ...
    public function __construct($config)
    {
        ...
        // bets:
        $this->bet_help_message = $config['bet_help_message'];
    ...
    private function _process_user_message($username, $channel, $message_text)
    {
        ...
        elseif ($words_in_message_text[0] === $this->bot_config['quote_keyword'])
        {
            ...
            $this->_bot_command_add_usage($this->bot_config['quote_keyword']);
            return TRUE;
        }elseif ($message_text === $this->bot_config['helpcommandbet_keyword'])
        {
            $this->_display_helpbet_command($username, $channel, $words_in_message_text, $message_text);
            // add the bot command to usage:
            $this->_bot_command_add_usage($this->bot_config['helpcommandbet_keyword']);
            return TRUE;
        }
        //
    ...
    //
    //----------------------------------------------------------------------------------
    private function _display_helpbet_command($username, $channel, $words_in_message_text, $message_text)
    {
        $message = $this->bet_help_message;
        //
        if ($this->_check_response_should_be_silenced($this->bot_config['helpcommandbet_keyword']) === FALSE)
        {
            $this->send_text_to_server('bot', 'PRIVMSG ' . $channel . ' :' . $message);
            $this->_add_command_to_bot_responses_last_date($this->bot_config['helpcommandbet_keyword']);
        }
        //
        return TRUE;
    }
